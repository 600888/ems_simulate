...
说明：P0 遗留的点级控制策略、确认、事件轮询和时间同步已在后续 P1 实施中闭环。

### 1.2 P1 实施状态（2026-08-29）

| 编号 | 状态 | 已落地内容 |
| --- | --- | --- |
| DNP3-P1-01 | 已完成 | 事件按响应精确确认、超时重发、溢出 IIN、UTC 时标、Enable/Disable 与 Null Unsolicited；当前库不支持 G11/G42，因此输出状态事件策略明确为默认不产生 |
| DNP3-P1-02 | 已完成 | 产品自动读取只负责静态/指定点刷新，Handler 生命周期任务只轮询 Class 1/2/3；启用未请求上报后自动降为 60 秒兜底 |
| DNP3-P1-03 | 已完成 | Delay Measure、G50V1 对时与 NEED_TIME、Cold/Warm Restart 延迟对象、范围 Freeze/Freeze Clear/No Ack |
| DNP3-P1-04 | 已完成 | 点级 JSON 配置持久化与旧库迁移，覆盖变体、事件类、死区、SBO/DO、CROB 脉冲、初始品质、事件与时标策略；新增/批量新增、复制和属性编辑链路保留配置 |
| DNP3-P1-05 | 已完成 | 缓存保存 flags/variation/index/timestamp/来源/接收时间；DNP3 品质与时标已并入测点“属性编辑”，客户端只读刷新、服务端可配置；支持 TTL 和断线通信丢失失效 |
| DNP3-P1-06 | 已完成 | RESET/状态请求、LINK_STATUS、Confirmed User Data、FCB/FCV、ACK/NACK、超时重发、广播限制及应用事件确认 |
| DNP3-P1-07 | 已完成 | 应用序号请求响应关联、SELECT→OPERATE 关联、fragment 关联、链路/IIN/限定词说明和扩展对象名；未知对象保留原始偏移并尝试恢复后续对象 |
| DNP3-P1-08 | 已完成（自动化） | Master↔Outstation 真实 TCP、拆包/粘包、事件、SBO/DO、链路确认、对时、冻结、断线恢复及固定线报文回归样本；真实第三方设备/工具联调保留为发布环境验证项 |

P1 自动化功能闭环已完成。第三方实机互操作依赖外部设备或独立工具环境，不作为本地测试伪造；发布前应使用实际目标设备或独立 DNP3 工具执行最终验收并归档抓包。
