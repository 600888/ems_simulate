...
class Slave(Base):
    ...
    slave_id: Mapped[int] = mapped_column(Integer, nullable=False, comment="从机地址(0-247)")
    name: Mapped[str | None] = mapped_column(String(64), nullable=True, comment="从机名称")
    enable: Mapped[bool] = mapped_column(Boolean, server_default="1", comment="是否启用")

    # 关系
    channel = relationship("Channel", back_populates="slaves")

    __table_args__ = (UniqueConstraint("channel_id", "slave_id", name="uq_slave_channel_slave"), {"comment": "从机表"})
