            ...
            try_methods = [
                ...
                ("ref_None",
                 lambda: iec61850.IedConnection_getDataSetDirectory(
                     self._connection.connection, mms_ref, None)),
                # (conn, ref, c_bool()) - 传 c_bool 对象
                ("ref_cbool",
                 lambda: iec61850.IedConnection_getDataSetDirectory(
                     self._connection.connection, mms_ref, c_bool())),
            ]
            ...
            def _extract_entry_as_fcda(node, ll_data=None) -> dict:
                ...
                for ld_attr, var_attr, comp_attr in attr_tries:
                    ...
                    if ld_name and var_name:
                        ...
                        log.debug(f"DataSetEntry 提取成功: attr_variant={ld_attr}/{var_attr}/{comp_attr}, "
            ...
            while it:
                ...
                if member:
                    ...
                    try:
                        ...
                        str_val = "<str fails>"
                    log.debug(f"DataSet {dataset_ref}: 节点 {node_count} 提取失败, "
                               f"data_type={data_type_name}, str={str_val}")
