...
from proto.iec61850.model.ied_model import (
    IedModel, LDModel, LNModel, DORef, DARef, DataSetRef
)
...
class TestIcdExporter:
    ...
    @pytest.mark.parametrize("ln_name,expected", [
        ("LLN0", "LLN0"),
        ("MMCL", "MMCL"),
        ("MMCL1", "MMCL"),
        ("MMBC", "MMBC"),
    ...
    @pytest.mark.parametrize("ld_name,ied_name,expected", [
    ...
    def test_do_type_fingerprint_sharing(self):
        """相同 DA 结构的 DO 应有相同指纹"""
        do1 = DORef(name="Vol", ref="LD1/MMXU1.Vol", cdc="MV", frame_type=0, das=(
        ...
        do2 = DORef(name="Amp", ref="LD1/MMXU1.Amp", cdc="MV", frame_type=0, das=(
            DARef(name="mag", path="mag.f", fc="MX", iec_type=IecType.FLOAT),
            DARef(name="q", path="q", fc="MX", iec_type=IecType.INTEGER),
            DARef(name="t", path="t", fc="MX", iec_type=IecType.TIMESTAMP),
        ))
    ...
    def test_do_type_fingerprint_different(self):
        ...
        do2 = DORef(name="Pos", ref="LD1/CSWI1.Pos", cdc="DPC", frame_type=2, das=(
    ...
    # ===== CDC 推断 =====
    @pytest.mark.parametrize("do_name,ln_class,expected", [
        ("Vol", "MMXU", "MV"),
        ("Amp", "MMXU", "MV"),
        ("Hz", "MMXU", "MV"),
        ("PhV", "MMXU", "CMV"),
        ...
        model = IedModel(
            ...
            lds=(
                LDModel(
                    ...
                    lns=(
                        LNModel(
                            name="LLN0", ln_class="LLN0", ref="IED1_CTRL1/LLN0",
                            dos=(), datasets=(), rcb_list=(), gocb_list=(),
                        ),
                        LNModel(
                            name="CSWI1", ln_class="CSWI", ref="IED1_CTRL1/CSWI1",
                            dos=(
                                DORef(name="Pos", ref="IED1_CTRL1/CSWI1.Pos", cdc="DPC", frame_type=2, das=(
                                    DARef(name="stVal", path="stVal", fc="ST", iec_type=IecType.INTEGER),
                                    DARef(name="q", path="q", fc="MX", iec_type=IecType.INTEGER),
                                    DARef(name="t", path="t", fc="MX", iec_type=IecType.TIMESTAMP),
                                )),
                            ),
                            datasets=(), rcb_list=(), gocb_list=(),
                        ),
                    ),
                ),
            ),
    ...
    def test_build_datasets_do_matching(self):
        ...
        discovered_lns = (
            LNModel(
                name="MMCL1", ln_class="MMCL", ref="LD1/MMCL1",
                dos=(
                    DORef(name="Temp001", ref="LD1/MMCL1.Temp001", cdc="MV", frame_type=0, das=(
                        DARef(name="mag", path="mag.f", fc="MX", iec_type=IecType.FLOAT),
                    )),
                    DORef(name="Temp002", ref="LD1/MMCL1.Temp002", cdc="MV", frame_type=0, das=(
                        DARef(name="mag", path="mag.f", fc="MX", iec_type=IecType.FLOAT),
                    )),
                ),
                datasets=(), rcb_list=(), gocb_list=(),
        ...
        datasets = [
            DataSetRef(name="dsTemp", ref="LD1/LLN0.dsTemp", is_deletable=False, members=(
                {"ref": "LD1/L1.Temp001.mag.f", "fc": "MX", "iec_type": "float", "index": 0},
                {"ref": "LD1/L1.Temp002.mag.f", "fc": "MX", "iec_type": "float", "index": 0},
            )),
        ]
        ln = LNModel(
            name="LLN0", ln_class="LLN0", ref="LD1/LLN0",
            dos=(), datasets=datasets, rcb_list=(), gocb_list=(),
        )
    ...
    def test_mag_struct_da_uses_discovered_bdas(self):
        ...
        do = DORef(
            ...
            cdc="MV", frame_type=0,
            das=(
                DARef(name="mag", path="mag.f", fc="MX", iec_type=IecType.FLOAT, sub_das=(
                    DARef(name="f", path="mag.f", fc="MX", iec_type=IecType.FLOAT),
                )),
                DARef(name="q", path="q", fc="MX", iec_type=IecType.INTEGER),
                DARef(name="t", path="t", fc="MX", iec_type=IecType.TIMESTAMP),
            ),
        ...
        self.exporter._resolve_or_create_do_type(
            do, "MV", "KG_BAMSCTMP01.MMCL1",
            do_type_cache, da_type_cache,
        ...
        assert bda_names == {"f"}, (
            f"在线发现 mag.f 时 DAType 应只含 f，实际: {bda_names}"
        )
    ...
    def test_mag_struct_da_different_bda_different_dotype(self):
        ...
        # DO1: mag.f (如温度测量)
        do_f = DORef(
            name="Temp001", cdc="MV", frame_type=0,
            das=(
                DARef(name="mag", fc="MX", iec_type=IecType.FLOAT, sub_das=(
                    DARef(name="f", fc="MX", iec_type=IecType.FLOAT),
                )),
        ...
        # DO2: mag.i (如状态计数)
        do_i = DORef(
            name="SglMaxVolNo", cdc="MV", frame_type=0,
            das=(
                DARef(name="mag", fc="MX", iec_type=IecType.INTEGER, sub_das=(
                    DARef(name="i", fc="MX", iec_type=IecType.INTEGER),
                )),
                DARef(name="q", fc="MX", iec_type=IecType.INTEGER),
            ),
        )
