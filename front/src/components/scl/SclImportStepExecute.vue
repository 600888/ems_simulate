  ...
  <div class="import-step-execute">
    ...
    <div v-else class="result-section">
      <el-result
        ...
        :title="
          result.success ? $t('scl.importSuccess') : $t('scl.importFailed')
        "
      >
        <template #extra>
          <div class="result-stats">
            <p>
              {{ $t("scl.resultYC") }}: {{ result.yc }} |
              {{ $t("scl.resultYX") }}: {{ result.yx }} |
              {{ $t("scl.resultYK") }}: {{ result.yk }} |
              {{ $t("scl.resultYT") }}: {{ result.yt }}
            </p>
            <p v-if="result.goose_count">GOOSE: {{ result.goose_count }}</p>
            <p v-if="result.report_count">
              {{ $t("scl.resultReports") }}: {{ result.report_count }}
            </p>
            <div v-if="result.errors.length" class="result-errors">
              <p v-for="(e, i) in result.errors" :key="i" class="error-item">
                ❌ {{ e }}
              </p>
            </div>
          </div>
