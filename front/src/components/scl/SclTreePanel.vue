    ...
    <div class="tree-wrapper">
      ...
      >
        <template #default="{ node, data }">
          <span class="tree-node-wrapper" :class="{ 'diff-highlight': isDiffHighlight(data) }">
            <span class="tree-node-icon">{{ getNodeIcon(data) }}</span>
            <span class="tree-node-label">{{ data.label }}</span>
            <span v-if="data.badge" class="tree-node-badge">{{ data.badge }}</span>
          </span>
        </template>
      </el-tree>
    </div>
...
.tree-node-label { font-size: 13px; white-space: nowrap; color: var(--text-primary); }
.tree-node-badge {
  font-size: 11px;
