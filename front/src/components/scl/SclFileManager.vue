  ...
  <div class="scl-file-manager">
    ...
    >
      ...
      >
        <template #default="{ row }">
          ...
          <div class="file-summary" v-if="row.ied_name">
            IED: {{ row.ied_name }}
            <template v-if="row.ycCount !== undefined">
              | YC={{ row.ycCount }} YX={{ row.yxCount }} YK={{
                row.ykCount
              }}
              YT={{ row.ytCount }}</template
            >
          </div>
        </template>
      </el-table-column>
      <el-table-column :label="$t('scl.fileType')" width="80" align="center">
        <template #default="{ row }">
          <el-tag
            :type="
              (row.file_type ||
                row.extension?.replace('.', '').toUpperCase()) === 'ICD'
                ? 'success'
                : 'warning'
            "
            size="small"
      ...
      <el-table-column :label="$t('scl.fileSize')" width="100" align="right">
        <template #default="{ row }">{{
          row.size_display || formatSize(row.file_size)
        }}</template>
      </el-table-column>
      <el-table-column :label="$t('scl.uploadTime')" width="160" align="center">
        <template #default="{ row }">{{
          row.upload_time || row.modified_time || "-"
        }}</template>
      </el-table-column>
      <el-table-column
        :label="$t('scl.operations')"
        width="380"
        fixed="right"
        align="center"
      >
        <template #default="{ row }">
          <el-button-group>
            <el-button type="primary" link @click.stop="handlePreview(row)">{{
              $t("scl.previewAction")
            }}</el-button>
            <!-- 导入功能尚未完成，暂时隐藏
            <el-button type="success" link @click.stop="handleImport(row)">{{ $t('scl.importAction') }}</el-button>
            -->
            <el-button type="warning" link @click.stop="handleXmlView(row)">{{
              $t("scl.xmlView")
            }}</el-button>
            <el-button type="danger" link @click.stop="handleDelete(row)">{{
              $t("scl.deleteAction")
            }}</el-button>
            <el-button type="info" link @click.stop="handleDiff(row)">{{
              $t("scl.diffAction")
            }}</el-button>
          </el-button-group>
        </template>
...
async function loadFiles() {
  ...
  try {
    ...
    files.value = (raw || []).map((f: any) => ({
      ...
      file_type:
        f.file_type || (f.extension || "").replace(".", "").toUpperCase(),
    }));
  } catch {
    files.value = [];
  } finally {
