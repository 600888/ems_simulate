<template>
  ...
  <div v-else class="control-panel">
    <el-descriptions :column="2" border size="small" label-width="auto">
      ...
      <el-descriptions-item label="控制块引用 (GoCBRef)" :span="2">{{ block.go_cb_ref }}</el-descriptions-item>
      ...
      <el-descriptions-item label="数据集 (DatSet)" :span="2">{{ block.data_set_ref || '-' }}</el-descriptions-item>
      <el-descriptions-item label="IED/逻辑设备/逻辑节点 (IED/LD/LN)">{{ block.ied_name }} / {{ block.ld_inst }} / {{ block.ln_name }}</el-descriptions-item>
      ...
      <el-descriptions-item label="最后更新时间 (LastUpdate)">{{ formatGooseTime(block.last_update) }}</el-descriptions-item>
      <el-descriptions-item label="报文数量 (Messages)">{{ block.message_count || 0 }}</el-descriptions-item>
      <el-descriptions-item label="存活时间 (TimeAllowedToLive)">{{ block.subscription?.time_allowed_to_live || 0 }} ms</el-descriptions-item>
    </el-descriptions>
...
function apply() { emit('apply', { ...form, app_id: parseAppId(form.app_id) }); }
</script>
