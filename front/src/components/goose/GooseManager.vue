<template>
  <div class="goose-manager">
    ...
    <!-- 创建 Publisher 对话框 -->
    <el-dialog
      v-model="createPublisherVisible"
      :title="configEditingId ? $t('common.edit') : $t('goose.newPublisher')"
      width="600px"
      destroy-on-close
    >
      <el-form
      ...
      >
        <el-form-item :label="$t('goose.goCbRef')" prop="go_cb_ref">
          <el-input
            v-model="publisherForm.go_cb_ref"
            :placeholder="$t('goose.createPublisherPlaceholder')"
          />
        </el-form-item>
        <el-form-item :label="$t('goose.goId')" prop="go_id">
          <el-input
            v-model="publisherForm.go_id"
            :placeholder="$t('goose.goIdPlaceholder')"
          />
        </el-form-item>
        <el-form-item :label="$t('goose.dataSetRef')" prop="data_set_ref">
          <el-input
            v-model="publisherForm.data_set_ref"
            :placeholder="$t('goose.dataSetRefPlaceholder')"
          />
        </el-form-item>
        <el-form-item :label="$t('goose.interface')" prop="interface">
          <el-select
            v-model="publisherForm.interface"
        ...
        <el-row :gutter="16">
          <el-col :span="12">
            <el-form-item
              :label="$t('goose.timeAllowedToLive')"
              prop="time_allowed_to_live"
            >
              <el-input-number
                v-model="publisherForm.time_allowed_to_live"
                :min="100"
                :max="60000"
                :step="100"
                style="width: 100%"
          ...
          <el-col :span="12">
            <el-form-item :label="$t('goose.confRev')" prop="conf_rev">
              <el-input-number
                v-model="publisherForm.conf_rev"
                :min="1"
                style="width: 100%"
              />
            </el-form-item>
        ...
        <el-row :gutter="16">
          ...
          <el-col :span="12">
            <el-form-item :label="$t('goose.vlanPrio')">
              <el-input-number
                v-model="publisherForm.vlan_prio"
                :min="0"
                :max="7"
                style="width: 100%"
              />
        ...
        <el-form-item :label="$t('goose.entryList')">
          <div class="entry-list">
            <div
              v-for="(entry, idx) in publisherForm.entries"
              :key="idx"
              class="entry-row"
            >
              <el-input
                v-model="entry.name"
                :placeholder="$t('goose.entryNamePlaceholder')"
                style="width: 120px"
              />
              <el-select v-model="entry.iec_type" style="width: 130px">
                <el-option
                  v-for="opt in GOOSE_IEC_TYPE_OPTIONS"
                  :key="opt.value"
                  :label="opt.label"
                  :value="opt.value"
                />
              ...
              />
              <el-input-number
                v-else-if="entry.iec_type === 'float'"
                v-model="entry.value"
                :controls="false"
                :precision="2"
                style="width: 100px"
              ...
              <el-button
                ...
                circle
                size="small"
                @click="publisherForm.entries.splice(idx, 1)"
              />
            </div>
            <el-button :icon="Plus" size="small" @click="addPublisherEntry">{{
              $t("goose.addEntry")
            }}</el-button>
          </div>
        </el-form-item>
    ...
    <!-- 创建 Receiver 对话框 -->
    <el-dialog
      v-model="createReceiverVisible"
      :title="receiverEditingId ? $t('common.edit') : $t('goose.newReceiver')"
      width="500px"
      destroy-on-close
    >
      <el-form :model="receiverForm" label-width="100px">
        ...
        <el-form-item :label="$t('goose.iface')" required>
          <el-select
            v-model="receiverForm.interface"
            :placeholder="$t('goose.interfacePlaceholder')"
            style="width: 100%"
          >
            <el-option
        ...
        <el-form-item :label="$t('goose.subscriptions')">
          <div class="entry-list">
            <div
              v-for="(sub, idx) in receiverForm.subscriptions"
              :key="idx"
              class="entry-row"
            >
              <el-input
                v-model="sub.go_cb_ref"
                :placeholder="$t('goose.subPlaceholder')"
                style="flex: 1"
              />
              <el-input-number
                v-model="sub.app_id"
                :min="0"
                :max="65535"
                :placeholder="$t('goose.appIdPlaceholder')"
                style="width: 120px"
              />
              <el-button
                type="danger"
    ...
    <el-dialog
      ...
      :title="$t('goose.dataSet') + ' - ' + (editingPublisher?.go_cb_ref || '')"
      width="700px"
      destroy-on-close
    >
      <el-table :data="editingEntries" border size="small">
        ...
        <el-table-column :label="$t('goose.entryValue')" min-width="150">
          <template #default="{ row }">
            <el-switch
              v-if="row.iec_type === 'boolean'"
              v-model="row.value"
              @change="onEntryValueChange(row)"
            />
            <el-input-number
              v-else-if="row.iec_type === 'integer'"
              v-model="row.value"
              size="small"
              @change="onEntryValueChange(row)"
            />
            <el-input-number
              v-else-if="row.iec_type === 'float'"
              v-model="row.value"
              :precision="2"
              size="small"
              @change="onEntryValueChange(row)"
      ...
      <div style="margin-top: 12px; display: flex; gap: 8px">
        <el-button
          ...
          size="small"
          :disabled="editingPublisher?.is_running"
          @click="addEntryToEditor"
          >{{ $t("goose.addEntry") }}</el-button
        >
      </div>
      <template #footer>
        ...
        <el-button
          type="primary"
          :disabled="editingPublisher?.is_running"
          @click="saveNewEntries"
          :loading="savingEntries"
          >{{ $t("goose.saveEntries") }}</el-button
        >
      </template>
    </el-dialog>
    ...
    >
      <div class="tab-header">
        <el-button
          type="primary"
          :icon="Plus"
          @click="showAddSubscriptionForm = true"
          v-if="!editingReceiver?.is_running"
        >
          {{ $t("goose.addSub") }}
        </el-button>
      ...
      >
        <el-form :inline="true">
          <el-form-item :label="$t('goose.subGoCbRef')">
            <el-input
              v-model="newSubForm.go_cb_ref"
              :placeholder="$t('goose.subGoCbRefPlaceholder')"
              style="width: 280px"
            />
          </el-form-item>
          <el-form-item :label="$t('goose.subAppId')">
            <el-input-number
              v-model="newSubForm.app_id"
              :min="0"
              :max="65535"
              :style="{ width: '140px' }"
          ...
          <el-form-item>
            ...
            <el-button @click="showAddSubscriptionForm = false">{{
              $t("goose.cancel")
            }}</el-button>
          </el-form-item>
        </el-form>
      ...
      <template v-if="editingReceiver?.subscriptions?.length">
        <div
          v-for="sub in editingReceiver.subscriptions"
          :key="sub.go_cb_ref"
          class="sub-card"
        >
          ...
          <div class="sub-card__row">
            ...
            <span class="sub-card__value">{{
              sub.app_id != null
                ? "0x" + sub.app_id.toString(16).toUpperCase().padStart(4, "0")
                : "-"
            }}</span>
          ...
          <div class="sub-card__row">
            ...
            <span class="sub-card__label" style="margin-left: 20px"
              >{{ $t("goose.subDataValue") }}:</span
            >
            <span class="sub-card__value">
              <template v-if="sub.data_values?.length">
                <span
                  v-for="dv in sub.data_values"
                  :key="dv.index"
                  class="data-value-item"
                  >{{ dv.value }}</span
                >
              </template>
      ...
      <span
        v-else
        class="text-muted"
        style="display: block; text-align: center; padding: 24px"
        >{{ $t("goose.noSubscription") }}</span
      >
    </el-dialog>
    ...
    <el-dialog
      ...
      :title="$t('goose.subscriptionDetail')"
      width="500px"
    >
      <el-descriptions :column="2" border v-if="selectedSubscription">
        <el-descriptions-item :label="$t('goose.goCbRef')" :span="2">{{
          selectedSubscription.go_cb_ref
        }}</el-descriptions-item>
        <el-descriptions-item :label="$t('goose.goId')">{{
          selectedSubscription.go_id || "-"
        }}</el-descriptions-item>
        ...
        <el-descriptions-item :label="$t('goose.dataSetRef')" :span="2">{{
          selectedSubscription.data_set_ref || "-"
        }}</el-descriptions-item>
        <el-descriptions-item :label="$t('goose.stNum')">{{
          selectedSubscription.st_num
        }}</el-descriptions-item>
        <el-descriptions-item :label="$t('goose.sqNum')">{{
          selectedSubscription.sq_num
        }}</el-descriptions-item>
        <el-descriptions-item :label="$t('goose.confRev')">{{
          selectedSubscription.conf_rev
        }}</el-descriptions-item>
        <el-descriptions-item :label="$t('goose.subState')">
          <el-tag
            :color="GOOSE_STATE_COLOR[selectedSubscription.state]"
            style="color: #fff"
            size="small"
          >
            {{
        ...
        <el-descriptions-item :label="$t('goose.timeAllowedToLive')">{{
          selectedSubscription.time_allowed_to_live
        }}</el-descriptions-item>
        <el-descriptions-item :label="$t('goose.dstMac')">{{
          selectedSubscription.dst_mac || "-"
        }}</el-descriptions-item>
        <el-descriptions-item :label="$t('goose.description')" :span="2">{{
...
import {
  ...
  getGooseReceivers,
  getDiscoveredGoose,
  importDiscoveredGoose,
  createGoosePublisher,
  parseGooseAppId,
...
const editingEntries = ref<
  { name: string; value: any; iec_type: string; _new?: boolean }[]
>([]);
...
async function saveNewEntries() {
  ...
  if (
    new Set(editingEntries.value.map((entry) => entry.name)).size !==
    editingEntries.value.length
  ) {
...
async function addSubscription() {
  ...
  try {
    if (editingSubRef.value) {
      const subscriptions = editingReceiver.value.subscriptions.map((sub) =>
        sub.go_cb_ref === editingSubRef.value
          ? {
...
<style lang="scss">
/* 网络接口选择器下拉选项样式（非 scoped，因为 el-option 的下拉面板渲染在 body 挂载点） */
