        ...
        <el-form :model="form" :label-width="i18nLabelWidth">
          ...
          <el-form-item :label="$t('pointMapping.sourcePoint')">
            <div class="source-points-container">
              <div
                v-for="(item, index) in form.source_point_codes"
                :key="index"
                class="source-item"
              >
                <el-tree-select
                  ...
                  <template #default="{ node, data }">
                    <div class="custom-tree-node">
                      ...
                      <div class="node-content">
                        ...
                        <span v-if="data.type" class="node-tag">{{
                          data.type
                        }}</span>
                      </div>
                    </div>
                ...
                <el-button
                  ...
                  v-if="form.source_point_codes.length > 1"
                >
    ...
    const processNode = (nodes: any[], parentDeviceName: string = "") => {
      return nodes.map((node) => {
        ...
        if (
          !node.type &&
          node.children &&
          node.children.some((c: any) => c.type)
        ) {
  ...
  try {
    ...
    if (Array.isArray(parsed) && parsed.length > 0) {
      if (typeof parsed[0] === "string") {
        form.source_point_codes = parsed.map((c: string) => ({
          point_code: c,
          alias: c,
        }));
      } else {
...
const addSource = () =>
  form.source_point_codes.push({ point_code: "", alias: "" });
const removeSource = (index: number) =>
  form.source_point_codes.splice(index, 1);
...
.point-mapping-config {
  ...
  background-color: var(
    --panel-bg,
    #fff
  ); /* Keep var for dark mode, default white */
  border-radius: 8px;
