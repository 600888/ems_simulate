...
> **设备未开启时也可以进入「模拟配置」并提前保存配置；只有开始模拟时才需要先开启设备。**
...
1. 点击工具栏上的"**模拟配置**"按钮；
2. 如需立即开始模拟，再开启设备并等待状态变为"运行中"。

![](images/sim-config-entry.png)

> 工具栏上的"模拟配置"按钮不受设备启停状态限制。
...
1. **左侧测点树**：按分组树形展示设备全部测点；IEC 61850 设备按 DataModel 的 LD → LN 层级展示；
   - 支持关键字**搜索**测点；
...
- **模拟方式（模拟方式下拉）**：无 / 定值 / 随机 / 自增 / 自减 / 正弦波 / 斜坡 / 脉冲；
- **步长（Step）**：数值变化步长（模拟方式为"无"或"定值"时步长不可用）；
- **定值**：选择"定值模拟"后填写，模拟运行时测点持续保持该数值。

> 💡 不同测点可设置不同的模拟方式，实现"局部差异化模拟"（如电压做正弦波模拟、电流做随机模拟）。

### 步骤 3：保存配置

点击对话框底部"保存"，配置立即应用到当前设备，与测点表格中的模拟设置保持一致。保存不会启动模拟；若模拟已在运行，后续模拟按新配置执行。每次打开对话框都会读取当前配置，在测点表格中保存的修改也会同步显示。
...
- 编辑设备配置或重建设备实例时，相同测点的模拟方式、步长、定值、模拟范围和选择状态会保留；后端程序重启后需要重新配置；
