...
点击“**批量读取**”后，前端只提交一次后台读取任务；后端将**连续地址**的测点合并读取（如 Modbus 一次读取多个寄存器），非连续地址自动拆分为多次。前端后续查询的是任务状态，不会通过定时请求重复触发设备读取。
...
批量/DataSet 模式设置的是**轮询周期**；逐点模式设置的是相邻测点请求的**点间隔**，完整一轮结束后至少等待 1 秒再开始下一轮。页面刷新只展示后端缓存，不会额外触发设备读取。
