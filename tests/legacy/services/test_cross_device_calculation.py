...
from src.enums.points import Yc

logging.basicConfig(
    level=logging.DEBUG,
    format="%(asctime)s - %(name)s - %(levelname)s - %(message)s",
    handlers=[logging.FileHandler("test.log", encoding="utf-8", mode="w"), logging.StreamHandler()],
)
...
class TestCrossDeviceCalculation(unittest.TestCase):
    def setUp(self):
        ...
        self.mock_dc.device_map = {"DeviceA": self.dev_a, "DeviceB": self.dev_b}
